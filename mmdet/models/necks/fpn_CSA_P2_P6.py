...
class high2low_attn(BaseModule):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., ws=1, sr_ratio=1.0, init_cfg=None): # sr_ratio: the resize ratio of high feature map
...
class ASFF(BaseModule):
    ...
    def forward(self, x_level_0, x_level_1, x_level_2, x_level_3):
        ...
        fused_out_reduced = x_level_0 * levels_weight[:,0:1,:,:]+\
                            ...
                            x_level_2 * levels_weight[:,2:3,:,:]+\
                            x_level_3 * levels_weight[:,3:,:,:]
...
class FPN_CSA(BaseModule):
    def __init__(self,
        ...
        if with_norm:
            ...
            self.fpna_p2_1x1 = nn.Sequential(
                *[nn.Conv2d(in_channels[0], out_channels, 1, bias=False), nn.BatchNorm2d(out_channels)])
        ...
        # add attention
        # self_attention
        self.self_p2 = self_attn(dim=out_channels, num_heads=8, ws=7, sr_ratio=16)
        ...
        self.h2l_p3_p2 = high2low_attn(dim=out_channels, num_heads=8, ws=7, sr_ratio=8)
        self.h2l_p4_p2 = high2low_attn(dim=out_channels, num_heads=8, ws=7, sr_ratio=4)
        self.h2l_p5_p2 = high2low_attn(dim=out_channels, num_heads=8, ws=7, sr_ratio=2)
        ...
        self.l2h_p2_p3 = low2high_attn(out_channels, out_channels, 2)
        self.l2h_p2_p4 = low2high_attn(out_channels, out_channels, 4)
        self.l2h_p2_p5 = low2high_attn(out_channels, out_channels, 8)
    ...
    def forward(self, inputs):
        ...
        p4 = self.fpna_p4_1x1(inputs[self.start_level + 2])
        p3 = self.fpna_p3_1x1(inputs[self.start_level + 1])
        p2 = self.fpna_p2_1x1(inputs[self.start_level])


        fpna_p2_out = self.p2_fusion(self.self_p2(p2), self.h2l_p3_p2(p2, p3), self.h2l_p4_p2(p2, p4), self.h2l_p5_p2(p2, p5))
        fpna_p3_out = self.p3_fusion(self.self_p3(p3), self.h2l_p4_p3(p3, p4), self.h2l_p5_p3(p3, p5), self.l2h_p2_p3(p2, p3))
        fpna_p4_out = self.p4_fusion(self.self_p4(p4), self.h2l_p5_p4(p4, p5), self.l2h_p2_p4(p2, p4), self.l2h_p3_p4(p3, p4))
        fpna_p5_out = self.p5_fusion(self.self_p5(p5), self.l2h_p2_p5(p2, p5), self.l2h_p3_p5(p3, p5), self.l2h_p4_p5(p4, p5))
        ...
        fpn_csa_out = [fpna_p2_out, fpna_p3_out, fpna_p4_out, fpna_p5_out, fpna_p6_out]
