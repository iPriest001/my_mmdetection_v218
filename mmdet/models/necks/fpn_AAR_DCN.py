...
class AdaptiveAngleConv(nn.Module):  # deformable convolution version
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None, angle_list=[0]):
        ...
        self.baseline_conv = nn.Conv2d(inc, outc, kernel_size=kernel_size, stride=1, padding=1, bias=bias)
    ...
    def forward(self, x):
        ...
        x_offset_90 = self._get_x_offset(x, N, h, w, self.angle_list[1])
        x_offset_135 = self._get_x_offset(x, N, h, w, self.angle_list[3])
        x_offset_180 = self._get_x_offset(x, N, h, w, self.angle_list[2])
        #x_offset_225 = self._get_x_offset(x, N, h, w, self.angle_list[5])
        #x_offset_270 = self._get_x_offset(x, N, h, w, self.angle_list[6])
        #x_offset_315 = self._get_x_offset(x, N, h, w, self.angle_list[7])

        y = self.baseline_conv(x)  # common convolution
        y_45 = F.conv2d(x_offset_45, self.baseline_conv.weight, bias=self.baseline_conv.bias, stride=self.kernel_size,
                        padding=self.baseline_conv.padding)
        y_90 = F.conv2d(x_offset_90, self.baseline_conv.weight, bias=self.baseline_conv.bias, stride=self.kernel_size,
                        padding=self.baseline_conv.padding)
        y_135 = F.conv2d(x_offset_135, self.baseline_conv.weight, bias=self.baseline_conv.bias, stride=self.kernel_size,
                         padding=self.baseline_conv.padding)
        y_180 = F.conv2d(x_offset_180, self.baseline_conv.weight, bias=self.baseline_conv.bias, stride=self.kernel_size,
                         padding=self.baseline_conv.padding)
        """
        y_225 = F.conv2d(x_offset_225, self.baseline_conv.weight, bias=self.baseline_conv.bias, stride=self.kernel_size,
                         padding=self.baseline_conv.padding)
        y_270 = F.conv2d(x_offset_270, self.baseline_conv.weight, bias=self.baseline_conv.bias, stride=self.kernel_size,
                         padding=self.baseline_conv.padding)
        y_315 = F.conv2d(x_offset_315, self.baseline_conv.weight, bias=self.baseline_conv.bias, stride=self.kernel_size,
                         padding=self.baseline_conv.padding)
        """
    ...
    def _get_x_offset(self, x, N, h, w, angle):  # result before common convolution
        dtype = x.data.type()
        b1, c1, h1, w1 = x.size()
        offset = self._get_p_offset(N, b1, angle, dtype)
        ks = self.kernel_size
        N = offset.size(1) // 2
    ...
    def _get_p_offset(self, N, b, angle, dtype):
        n = angle // 45
        sqrt_2 = 2**0.5
        """
        if n == 0:  # 0 degree
        ...
        if n == 5: # 225 degree
            offset_x = [1+sqrt_2, 1+sqrt_2*0.5, 1, sqrt_2*0.5, 0, -sqrt_2*0.5, -1, -1-sqrt_2*0.5, -1-sqrt_2]
            offset_y = [1, -sqrt_2*0.5, -1-sqrt_2, 1+sqrt_2*0.5, 0, -1-sqrt_2*0.5, 1+sqrt_2, sqrt_2*0.5, -1]

        if n == 6: # 270 degree
            offset_x = [2, 1, 0, 1, 0, -1, 0, -1, -2]
            offset_y = [0, -1, -2, 1, 0, -1, 2, 1, 0]

        if n == 7: # 315 degree
            offset_x = [1, 1-sqrt_2*0.5, 1-sqrt_2, sqrt_2*0.5, 0, -sqrt_2*0.5, sqrt_2-1, sqrt_2*0.5-1, -1]
            offset_y = [1-sqrt_2, -sqrt_2*0.5, -1, 1-sqrt_2*0.5, 0, sqrt_2*0.5-1, 1, sqrt_2*0.5, sqrt_2-1]
        ...
        offset = offset.view(1, 2 * N, 1, 1).type(dtype).repeat(b, 1, 1, 1)  # offset.view(1, 2 * N, 1, 1)
...
class FPN_AAR_DCN(nn.Module):
    def __init__(self,
                 ...
                 angle_nums=5):
        super(FPN_AAR_DCN, self).__init__()
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            fpn_rotate_conv = AdaptiveAngleConv(
                ...
                1,
                1,
                angle_list=[0, 90, 180, 270]  #, 225, 270, 315]
            )
